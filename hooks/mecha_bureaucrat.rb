...
test_results =	
	target_execution.tasks.map { |task|
		if not task.artifacts.find_by(name: 'task.json') then next end
	...
	[
		...
		"Report: "+ task.task_values.find_by(property_id: 18).value.value,
		"Setup: ",
		actors_info
	].join("\n\n")
}.join("\n\n")
...
description =
	[
		": Mechatouch testing for commit #{workitem["commit_sha"]} has been completed and test execution can be found at #{execution_uri}. Test results and reports are given below.",
		test_results,
	].join("\n\n")

#Merge/Append results to incoming workitem.
if workitem["payload"]["results"]
	workitem["payload"]["results"].append({"label": "Mechatouch","vote": 1,"reported": false,"type":"Testing","desc": description})
else
	workitem["payload"]["results"] = [{"label": "Mechatouch","vote": 1,"reported": false,"type":"Testing","desc": description}]
end
